...
function normalizeKecamatan($kecamatan) {
    // Handle variasi nama kecamatan Klampok
...
echo "Memulai import data perangkat desa untuk Kecamatan Klampok dari file asli...\n";
...
if (($handle = fopen($csvFile, "r")) !== FALSE) {
    ...
    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
        ...
        // Filter hanya untuk Kecamatan Klampok (termasuk variasi nama)
        if (normalizeKecamatan($kecamatan) !== 'klampok') {
            continue;
        }
...
// Tampilkan desa di Kecamatan Klampok yang tidak memiliki perangkat
echo "\n=== DESA TANPA PERANGKAT DI KECAMATAN KLAMPOK ===\n";
$stmt = $conn->prepare("
    SELECT d.nama_desa 
    FROM desa d 
    LEFT JOIN perangkat_desa pd ON d.id = pd.desa_id 
    WHERE LOWER(d.kecamatan) = 'klampok' AND pd.id IS NULL
...
if (!empty($desaTanpaPerangkat)) {
    ...
    echo "Semua desa di Kecamatan Klampok sudah memiliki perangkat.\n";
}
