...
// File CSV yang akan diproses untuk Kecamatan Rakit
$csvFiles = [
...
foreach ($csvFiles as $csvFile) {
    ...
    if (($handle = fopen($csvFile, "r")) !== FALSE) {
        ...
        while (($data = fgetcsv($handle)) !== FALSE) {
            ...
            // Filter hanya untuk Kecamatan Rakit
            if (normalizeKecamatan($namaKecamatan) !== 'rakit') {
                continue;
            }
            ...
            try {
                ...
                $stmt = $pdo->prepare("
                    ...
                    (desa_id, nama_lengkap, tempat_lahir, alamat, no_telepon, pendidikan, tahun_diangkat, no_sk, jabatan, created_at, updated_at) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                ");
...
echo "\n=== RINGKASAN IMPORT KECAMATAN RAKIT ===\n";
...
$stmt = $pdo->prepare("SELECT nama_desa FROM desa WHERE LOWER(kecamatan) = 'rakit' ORDER BY nama_desa");
$stmt->execute();
