...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $existing = $db->select(
            "SELECT id FROM kategori_produk WHERE nama_kategori = ? AND id != ?",
    ...
    if (empty($errors)) {
        try {
            $db->execute(
                "UPDATE kategori_produk SET nama_kategori = ?, deskripsi = ? WHERE id = ?",
                [$nama_kategori, $deskripsi, $kategori_id]
            );
...
$produk_stats = $db->select(
    "SELECT COUNT(*) as total_produk FROM produk WHERE kategori_id = ?",
    [$kategori_id]
)[0];
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Edit Kategori Produk - Sistem Manajemen Desa</title>
...
<body class="bg-gray-50 font-sans">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 flex items-center">
                        <i class="fa fa-edit text-primary-600 mr-3"></i>
                        Edit Kategori Produk
                    </h1>
                    <p class="text-sm text-gray-600 mt-1">Perbarui informasi kategori produk</p>
                </div>
            ...
            <div class="lg:col-span-1">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
                    ...
                    <div class="p-6">
                        <div class="space-y-4">
                            <div>
                                <h4 class="text-sm font-medium text-gray-900 mb-1">Nama Kategori</h4>
                                <p class="text-sm text-gray-600"><?= htmlspecialchars($kategori['nama_kategori']) ?></p>
                            </div>
                            
                            <div>
                                <h4 class="text-sm font-medium text-gray-900 mb-1">Deskripsi</h4>
                                <p class="text-sm text-gray-600"><?= htmlspecialchars($kategori['deskripsi'] ?: 'Tidak ada deskripsi') ?></p>
                            </div>
                            
                            <div>
                                <h4 class="text-sm font-medium text-gray-900 mb-1">Jumlah Produk</h4>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    <?= number_format($produk_stats['total_produk']) ?> produk
                                </span>
                            </div>
                            
                            <div>
                                <h4 class="text-sm font-medium text-gray-900 mb-1">Tanggal Dibuat</h4>
                                <p class="text-sm text-gray-600"><?= date('d/m/Y H:i', strtotime($kategori['created_at'])) ?></p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                    <div class="px-6 py-4 border-b border-gray-200">
                        <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                            <i class="fa fa-lightbulb text-yellow-600 mr-2"></i>
                            Tips
                        </h3>
                    </div>
                    <div class="p-6">
                        <div class="space-y-4">
                            <div class="flex items-start">
                                ...
                                <div>
                                    <h4 class="text-sm font-medium text-gray-900 mb-1">Perhatian</h4>
                                    <p class="text-sm text-gray-600">Mengubah nama kategori akan mempengaruhi semua produk yang menggunakan kategori ini.</p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start">
                                ...
                                <div>
                                    <h4 class="text-sm font-medium text-gray-900 mb-1">Keamanan Data</h4>
                                    <p class="text-sm text-gray-600">Kategori yang memiliki produk tidak dapat dihapus untuk menjaga integritas data.</p>
                                </div>
                            </div>
                            
                            <div class="flex items-start">
                                <i class="fa fa-search text-green-500 mt-1 mr-3"></i>
                                <div>
                                    <h4 class="text-sm font-medium text-gray-900 mb-1">Pencarian</h4>
                                    <p class="text-sm text-gray-600">Gunakan nama yang mudah dicari dan dipahami oleh pengguna lain.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-sm text-gray-500" id="previewDesc">
                                        <?= htmlspecialchars($kategori['deskripsi'] ?: 'Deskripsi kategori') ?>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.getElementById('nama_kategori').addEventListener('input', function() {
            const value = this.value || '<?= htmlspecialchars($kategori['nama_kategori']) ?>';
            document.getElementById('previewName').textContent = value;
        });

        document.getElementById('deskripsi').addEventListener('input', function() {
            const value = this.value || '<?= htmlspecialchars($kategori['deskripsi'] ?: 'Deskripsi kategori') ?>';
